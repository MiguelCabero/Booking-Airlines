...
class LogsapiApplicationTests {
	...
	@Test
	@Order(4)
	public void updateByIdAndTimeStampReturnsOk()
			throws InterruptedException {

		final String timeStampEnd = Timestamp
		...
		final String responseString = given()
				.header("Content-type", "application/json").when()
				.body(timeStampUpdate).put(resource).then().log()
				.all().assertThat().statusCode(200).extract()
				.asString();
	}
}
