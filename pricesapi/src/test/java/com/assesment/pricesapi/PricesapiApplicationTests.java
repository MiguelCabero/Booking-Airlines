...
public class PricesapiApplicationTests {
	...
	public void generateFlightsWhenCalledReturnsOk() {
		final String date_target = "2022-11-17";
		final String resource = "http://localhost:8083/api/prices/1/2/" + date_target;

		final String responseString = given().header("Content-type", "application/json")
				.header("Accept", "application/json").get(resource).then().log().all().assertThat().statusCode(200)
				.extract().asString();
	}
