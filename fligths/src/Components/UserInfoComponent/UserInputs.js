...
function UserInputs(props) {
  ...
  useEffect(() => {
    currentPassengersContext.passengers.forEach((passenger) => {
		console.log('hola');
      axios
        .post(
          "http://localhost:8083/api/prices",
          {
            cityOne: currentTripContext.trips[0].selectedOrigin.id,
            cityTwo: currentTripContext.trips[0].selectedDestination.id,
            airline: currentTripContext.trips[0].selectedCompany,
            luggage: passenger.bags,
            layover: currentTripContext.trips[0].layover,
            date_selected: currentTripContext.trips[0].selectedDate,
            age_of_passenger: passenger.age,
          },
          {
            headers: {
        ...
        .then((response) => {
          finalPrices.push(response.data.price);
		  currentTrips[0].finalPrices = finalPrices;
  ...
  return (
    <div className="passenger-inputs-field">
      <div className="input-element">
        ...
        <input type="text" name={`user-name${props.index}`} ref={nameRef} />
      </div>
      <div className="input-element">
      ...
      <div>
        <button onClick={handleDelete} value={props.index}>
          Delete
        </button>
      </div>
    </div>
